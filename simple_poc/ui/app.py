...
from simple_poc.tracking.tracker import (
    PersonTracker,
)  # Assumes tracker.py is correctly placed
from simple_poc.ui.gallery import (
    create_gallery_html,
)  # Assumes gallery.py is correctly placed
from simple_poc.tracking.map import (
    compute_homography,
    create_map_visualization,
)  # Assumes map.py is correctly placed
...
class MTMMCTrackerApp:
            ...
            for person_id, x, y, w, h in camera_gt[frame_id_to_check]:
                if w > 0 and h > 0:
                    ...
                    try:
                        ...
                        print(f"Error drawing GT box {person_id} cam {camera_id}: {e}")
        return frame
    ...
    def _ensure_homography(self, frame):
        ...
        if self.H is None and frame is not None and frame.size > 0:
            ...
            if self.frame_height and self.frame_width:
                print(
                    f"Calculating homography ({self.frame_width}x{self.frame_height})"
                )
                try:
                    ...
                    if self.H is None:
                        raise ValueError("Homography compute failed")
                    ...
                    self.dst_points = None
                    if self.tracker:
                        self.tracker.H = None  # Clear in tracker too
        ...
        with gr.Blocks() as demo:
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    gr.Markdown(
                        f"**Model:** {self.model_type.upper()} (`{self.model_path}`)"
                    )
                    dataset_path = gr.Textbox(
                        label="Dataset Path (Scene Level)",
                        value="/Volumes/HDD/MTMMC/train/train/s01/",
                    )
                    mode_dropdown = gr.Dropdown(
                        ["Model Detection", "Ground Truth"],
                        label="Mode",
                        value=self.mode,
                    )
                    ...
                    frame_slider = gr.Slider(
                        minimum=0, maximum=100, step=1, value=0, label="Frame"
                    )
                    ...
                    with gr.Column(
                        visible=False
                    ):  # Hidden buttons for gallery interaction
                        track_buttons = {
                            i: gr.Button(f"Track {i}", elem_id=f"track_button_{i}")
                            for i in range(1, 500)
                        }  # Range for potential track IDs
                with gr.Column(scale=3):
                    with gr.Tabs():
                        with gr.TabItem("Tracking View"):
                            image_output = gr.Image(
                                label="Combined Camera View",
                                type="numpy",
                                image_mode="RGB",
                            )
                        with gr.TabItem("Map View"):
                            map_output = gr.Image(
                                label="Combined Top-Down Map",
                                type="numpy",
                                image_mode="RGB",
                            )
            ...
            mode_dropdown.change(
                self._on_mode_change, inputs=[mode_dropdown], outputs=[status_output]
            ).then(
            ...
            pause_checkbox.change(
                self._toggle_playback, inputs=[pause_checkbox], outputs=[status_output]
            )
            ...
            clear_btn.click(self._on_clear_selection, outputs=[status_output]).then(
                self._on_frame_change,
                inputs=[frame_slider],
                outputs=[image_output, map_output, gallery_output],
            )
    ...
    def _on_start(self, dataset_path, mode):
        ...
        print(
            f"Re-initializing tracker. Model: {self.model_type}, Path: {self.model_path}"
        )
        try:
            self.tracker = PersonTracker(
                self.model_path, self.model_type, self.map_width, self.map_height
            )
        except Exception as e:
        ...
        try:
            ...
            self.camera_ids = [
                os.path.basename(cam_dir) for cam_dir in self.camera_dirs
            ]
        except Exception as e:
        ...
        for cam_dir in self.camera_dirs:
            ...
            try:
                ...
                gt_load_errors.append(f"GT {cam_id}: {e}")
        ...
        if self.mode == "Ground Truth" and not found_gt and not gt_load_errors:
            status_msg += " Warning: No GT data found."
        ...
        if not initial_frames:
            return (
                "Error: Could not load initial frames.",
                gr.update(),
        ...
        first_valid_frame = next(
            (
                ...
                if initial_frames.get(cid) is not None and initial_frames[cid].size > 0
            ),
            None,
        )
        if first_valid_frame is None:
            return (
                "Error: All initial frames failed.",
                gr.update(),
                gr.update(value=True),
                None,
                None,
            ...
            if os.path.isdir(first_cam_rgb_dir):
                try:
                    max_frames = len(
                        [
                            f
                            for f in os.listdir(first_cam_rgb_dir)
                            if f.lower().endswith(".jpg")
                        ]
                    )
                except Exception as e:
                    status_msg += f" Warn: Max frames read error ({e})."
                    max_frames = 100
            else:
                status_msg += " Warn: RGB dir missing."
                max_frames = 100
        else:
            max_frames = 100

        # Process frame 0 for initial display
        output_image, map_img, gallery_html = self._process_and_get_outputs(
            0, initial_frames
        )
    ...
    def _process_and_get_outputs(
        self, frame_index: int, preloaded_frames: Optional[Dict[str, np.ndarray]] = None
        ...
        frames_bgr = (
            ...
            else self._load_frames(frame_index)
        )
        ...
        if not frames_bgr:
            ...
            map_placeholder_rgb = cv2.cvtColor(
                np.full((self.map_height, self.map_width, 3), 240, dtype=np.uint8),
                cv2.COLOR_BGR2RGB,
            )
        ...
        first_valid_frame = next(
            (f for f in frames_bgr.values() if f is not None and f.size > 0), None
        )
        ...
        if self.mode == "Model Detection":
            if not self.tracker:  # Check if tracker failed initialization
                ...
                raw_frames_rgb = {
                    cid: cv2.cvtColor(f, cv2.COLOR_BGR2RGB)
                    for cid, f in frames_bgr.items()
                    if f is not None
                }
                ...
                try:
                    ...
                    annotated_frames_rgb, map_img_model_rgb = (
                        self.tracker.process_multiple_frames(frames_bgr, self.paused)
                    )
                    output_image_rgb = self._combine_frames(
                        annotated_frames_rgb
                    )  # Combine annotated views
                    map_img_rgb = map_img_model_rgb  # Use the combined map returned
                    gallery_html = create_gallery_html(
                        self.tracker.person_crops, self.tracker.selected_track_id
                    )
                except Exception as e:
            ...
            for cam_id, frame_bgr in frames_bgr.items():
                ...
                if cam_id in self.gt_data and frame_id_to_check in self.gt_data[cam_id]:
                    for person_id, x, y, w, h in self.gt_data[cam_id][
                        frame_id_to_check
                    ]:
                        if w > 0 and h > 0:
                            ...
                            if len(current_gt_history_for_map[person_id]) > 30:
                                current_gt_history_for_map[person_id] = (
                                    current_gt_history_for_map[person_id][-30:]
                                )
            ...
            if self.H is not None and self.dst_points is not None:
                map_img_gt_bgr = create_map_visualization(
                    ...
                    all_gt_boxes_for_map,
                    all_gt_ids_for_map,
                    current_gt_history_for_map,
                    self.H,
                    None,
                )
                map_img_rgb = (
                    cv2.cvtColor(map_img_gt_bgr, cv2.COLOR_BGR2RGB)
                    if map_img_gt_bgr is not None
                    else None
                )
            else:
                placeholder_map = np.full(
                    (self.map_height, self.map_width, 3), 240, dtype=np.uint8
                )
                cv2.putText(
                    placeholder_map,
            ...
            output_image_rgb = self._combine_frames(
                annotated_frames_bgr
            )  # Combine annotated GT views
            if output_image_rgb is not None:
                output_image_rgb = cv2.cvtColor(
                    output_image_rgb, cv2.COLOR_BGR2RGB
                )  # Ensure final is RGB
    ...
    def _on_frame_change(self, frame_index):
        if self.dataset_path is None or not self.camera_dirs:
            return None, None, "<p>Dataset not loaded.</p>"
        try:
    ...
    def _on_clear_selection(self):
        if self.mode == "Model Detection" and self.tracker:
            return self.tracker.select_person(None)
        else:
            return "Clear selection only applicable in Model Detection mode (or tracker error)."
    ...
    def _on_refresh(self, frame_slider_value):
        ...
        output_image_rgb, map_img_rgb, gallery_html = self._process_and_get_outputs(
            frame_index, None
        )
    ...
    def _on_track_person(self, track_id):
        ...
        if self.mode == "Model Detection" and self.tracker:
            try:
                ...
                return self.tracker.select_person(
                    track_id_int
                )  # select_person handles logic
            except (ValueError, TypeError):
                return f"Error: Invalid track ID {track_id}"
            except Exception as e:
                return f"Error selecting person: {e}"
        else:
            return "Tracking selection only available in Model Detection mode (or tracker error)."
    ...
    def _load_frames(self, frame_index: int) -> Dict[str, Optional[np.ndarray]]:
        ...
        if self.camera_dirs:  # Get max frames estimate
            try:
                max_frames = len(
                    [
                        f
                        for f in os.listdir(os.path.join(self.camera_dirs[0], "rgb"))
                        if f.lower().endswith(".jpg")
                    ]
                )
            except Exception:
        ...
        h_placeholder = self.frame_height or 1080
        w_placeholder = self.frame_width or 1920
        ...
        for cam_dir in self.camera_dirs:
            ...
            try:
                if os.path.exists(image_path):
                    ...
                    if img is not None and (
                        self.frame_height is None or self.frame_width is None
                    ):
                        self.frame_height, self.frame_width = img.shape[:2]
                        h_placeholder, w_placeholder = (
                            self.frame_height,
                            self.frame_width,
                        )
            except Exception as e:
            ...
            if img is not None:
                ...
                placeholder_img = np.zeros(
                    (h_placeholder, w_placeholder, 3), dtype=np.uint8
                )
                cv2.putText(
                    placeholder_img,
    ...
    def _combine_frames(
        self, frames_dict: Dict[str, Optional[np.ndarray]]
    ) -> Optional[np.ndarray]:
        """Combines multiple frames/maps into a grid, resizing if necessary."""
        if not frames_dict:
            return None
        valid_frames_data = {
            cid: f for cid, f in frames_dict.items() if f is not None and f.size > 0
        }
        ...
        try:
            target_h, target_w, chans = valid_frames[0].shape
            dtype = valid_frames[0].dtype
        except Exception:
            return np.zeros((100, 100, 3), dtype=np.uint8)
        combined_image = np.zeros(
            (rows * target_h, cols * target_w, chans), dtype=dtype
        )
        frame_idx = 0
        for i in range(rows):
            for j in range(cols):
                    ...
                    try:
                        if current_frame.shape != (target_h, target_w, chans):
                            current_frame = cv2.resize(
                                current_frame, (target_w, target_h)
                            )
                        combined_image[
                            i * target_h : (i + 1) * target_h,
                            j * target_w : (j + 1) * target_w,
                            :,
                        ] = current_frame
                    except Exception as e:
                        print(
                            f"Error placing frame {frame_idx} (Cam:{sorted_cam_ids[frame_idx]}) in grid: {e}"
                        )
                    frame_idx += 1
