...
model_order_by_time = model_avg_time.sort_values(by='Time (s)', ascending=True)['Model'].tolist()
...
for env in df['Environment'].unique():
    ...
    fig1.add_trace(go.Scatter(
        ...
        mode='markers',
        marker=dict(size=10, symbol=env_symbols[env], color=colors[f'{env} Ground Truth'], line=dict(width=1,color='DarkSlateGrey')),
...
fig1.update_layout(
    ...
    xaxis={'categoryorder':'array', 'categoryarray':model_order_by_time}, # Apply sorted order
    barmode='group',
...
model_color_map = {model: model_colors_list[i % len(model_colors_list)] for i, model in enumerate(model_order_by_time)}
...
for model in model_order_by_time: # Iterate in sorted order
    ...
    fig3.add_trace(go.Scatter(
        ...
        marker=dict(
            ...
            symbol=[env_symbols[env] for env in model_df['Environment']] # Symbol by environment
            ),
...
fig4 = make_subplots(rows=2, cols=1, subplot_titles=("Detected vs. Ground Truth", "Processing Time (s)"))
...
fig4.update_xaxes(categoryorder='array', categoryarray=model_order_by_time, row=1, col=1) # Apply order
fig4.update_xaxes(categoryorder='array', categoryarray=model_order_by_time, title_text="Model", row=2, col=1) # Apply order & title
