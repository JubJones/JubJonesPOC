...
class MultiCameraReIDTracker:
    # Modified __init__ to accept camera_ids and create multiple detectors
    def __init__(self,
                 detector_path: str,
                 reid_weights_path: str,
                 camera_ids: List[str], # Added camera_ids argument
                 device: torch.device):
        self.device = device
        ...
        try:
            ...
            print(f"FATAL ERROR loading detector instance: {e}")
            sys.exit(1)

        # Load Single ReID Model Instance (OSNet) - Shared across cameras
        try:
    ...
    # --- _update_track_states, _determine_reid_needs, _extract_features, _perform_reid, draw_annotations ---
    # --- These methods remain UNCHANGED as they operate on the processed results ---
    # --- and the external track_states, not directly on detector instances ---
    # --- (Copying them here for completeness, but no logic changes within them) ---

    def _update_track_states(self, current_detections: Dict[str, Results], current_frame_idx: int):
        """
        Updates the state of tracked objects based on current detections.
        Manages age, state ('new', 'tracked', 'lost'), and last seen info.
        Sets the initial 'needs_reid' flag for new/reactivated tracks.
        (No changes needed here)
        """
        current_time = time.time()
        seen_tracker_ids_this_frame = set()
        for camera_id, results in current_detections.items():
            ...
            for i, tracker_id in enumerate(tracker_ids):
                seen_tracker_ids_this_frame.add(tracker_id)
                bbox_xyxy = boxes_xyxy[i]
                if tracker_id not in self.track_states:
                    self.track_states[tracker_id] = {
                        'global_id': None, 'last_seen_frame': current_frame_idx,
                        'last_update_time': current_time, 'age': 1, 'state': 'new',
                        'last_embedding': None, 'last_camera': camera_id,
                        'last_bbox_xyxy': bbox_xyxy, 'needs_reid': True,
                        'history': deque([(bbox_xyxy[0] + bbox_xyxy[2]) / 2, bbox_xyxy[3]], maxlen=50)
                    }
                else:
                    ...
                    if previous_state == 'lost':
                        track['state'] = 'tracked'; track['age'] = 1; track['needs_reid'] = True
                    else:
                        track['state'] = 'tracked'; track['age'] += 1
        lost_track_ids = []
        for tracker_id, track in self.track_states.items():
            if tracker_id not in seen_tracker_ids_this_frame and track['state'] != 'lost':
                track['state'] = 'lost'; track['age'] = 0; lost_track_ids.append(tracker_id)
        ids_to_delete = [
            tid for tid, trk in self.track_states.items()
            if trk['state'] == 'lost' and (current_frame_idx - trk['last_seen_frame'] > MAX_TRACK_AGE_BEFORE_DELETE)
        ]
        for tracker_id in ids_to_delete:
            #print(f"[Frame {current_frame_idx}] Deleting stale track {tracker_id} (Global: {self.track_states[tracker_id]['global_id']})")
            if tracker_id in self.track_states: del self.track_states[tracker_id]

    def _determine_reid_needs(self, current_frame_idx: int):
        """
        Applies conditions to flag tracks in self.track_states that require Re-ID.
        Sets the 'needs_reid' flag.
        (No changes needed here)
        """
        for tracker_id, track in self.track_states.items():
            if track['needs_reid'] or track['state'] == 'lost': continue
            if track['global_id'] is None and track['age'] >= REID_TRIGGER_AGE:
                track['needs_reid'] = True; continue
            if track['global_id'] is not None and track['age'] > 0 and (track['age'] % PERIODIC_REID_INTERVAL == 0):
                track['needs_reid'] = True; continue

    def _extract_features(self, frames: Dict[str, np.ndarray]) -> Dict[int, np.ndarray]:
        """
        Extracts Re-ID features for tracks flagged with 'needs_reid'.
        Performs extraction in batches for efficiency using the single ReID model.
        (No changes needed here)
        """
        ...
        for tid, bbox, cam_id in tracks_to_extract:
            if frames.get(cam_id) is not None:
                bboxes_by_camera[cam_id].append(bbox.astype(np.float32)); tracker_ids_by_camera[cam_id].append(tid)
        extraction_time = 0.0; total_features_extracted = 0
        for cam_id, bboxes_list in bboxes_by_camera.items():
            t_start = time.time(); frame_bgr = frames.get(cam_id)
            ...
            try:
                ...
                t_end = time.time(); extraction_time += (t_end - t_start)
                if batch_features is not None and len(batch_features) == len(tracker_ids_by_camera[cam_id]):
                    total_features_extracted += len(batch_features)
                ...
                # elif batch_features is None: print(f"W: Feature extraction failed (None) for cam {cam_id}.")
                # else: print(f"W: Feature extraction mismatch for cam {cam_id}. Expected {len(tracker_ids_by_camera[cam_id])}, got {len(batch_features)}.")
            except Exception as e: print(f"E: Feature extraction call for cam {cam_id}: {e}")
        # if total_features_extracted > 0: print(f"Extracted {total_features_extracted} features in {extraction_time:.4f} seconds.")
        return features

    def _perform_reid(self, new_features: Dict[int, np.ndarray], current_frame_idx: int):
        """
        Compares newly extracted features against the gallery. Assigns GlobalPersonIDs.
        Updates the gallery and track states.
        (No changes needed here)
        """
            ...
            if valid_gallery_ids:
                ...
                if similarities:
                    ...
                    if max_similarity >= REID_SIMILARITY_THRESHOLD:
                        best_match_index = np.argmax(similarities); best_match_global_id = valid_gallery_ids[best_match_index]; best_match_score = max_similarity
            if best_match_global_id is not None:
                matched_global_id = best_match_global_id
                if track['global_id'] is not None and track['global_id'] != matched_global_id:
                     print(f"[F:{current_frame_idx}] Potential ID Switch: T:{tracker_id} (was G:{track['global_id']}) -> G:{matched_global_id} (S:{best_match_score:.3f})")
                ...
                if track['global_id'] is None:
                    ...
                    # print(f"[F:{current_frame_idx}] ReID New ID: T:{tracker_id} -> G:{new_global_id}")
            track['last_embedding'] = new_embedding; track['needs_reid'] = False
        # if match_counts > 0 or new_id_counts > 0: print(f"ReID results: {match_counts} matches, {new_id_counts} new IDs.")
    ...
    def process_frame_batch(self, frames: Dict[str, np.ndarray], frame_idx: int) -> Dict[str, Results]:
        ...
        Orchestrates tracking update, ReID triggering, feature extraction, and matching.
        """
        if not frames:
            return {}
        ...
        for cam_id, frame_bgr in frames.items():
            ...
            # --- Use the specific detector instance for this camera ---
            if cam_id not in self.detectors:
                 print(f"Warning: No detector instance found for camera {cam_id}. Skipping detection.")
                 current_detections[cam_id] = None
                 continue
            ...
            try:
                ...
                # Call track on the specific detector instance
                results = detector_instance.track(
                    frame_contiguous,
                    persist=True, # Persist helps internal tracker state *for this instance*
                ...
                current_detections[cam_id] = results[0] if results else None
                 ...
                 print(f"Error during tracking for cam {cam_id} using its instance: {e}")
                 current_detections[cam_id] = None
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(base_scene_path):
         print(f"FATAL ERROR: Scene directory not found: {base_scene_path}")
         sys.exit(1)
    ...
    for cam_id in SELECTED_CAMERAS:
        ...
        if os.path.isdir(rgb_dir):
            ...
            print(f"Warning: RGB directory not found for {cam_id} in scene {SELECTED_SCENE} at {rgb_dir}. Skipping this camera.")

    if not valid_cameras:
        print(f"FATAL ERROR: No valid camera directories found for cameras {SELECTED_CAMERAS} in scene {SELECTED_SCENE}.")
        sys.exit(1)
    ...
    try:
        ...
        print(f"FATAL ERROR: Could not list or find image files in {first_cam_dir}: {e}")
        sys.exit(1)

    # --- Initialize the Tracker (passing valid_cameras) ---
    # This will now create multiple detector instances
    tracker = MultiCameraReIDTracker(
        detector_path=DETECTOR_MODEL_PATH,
        reid_weights_path=REID_MODEL_WEIGHTS,
        camera_ids=valid_cameras, # Pass the list of cameras to track
    ...
    for frame_idx, current_filename in enumerate(image_filenames):
        ...
        for cam_id in valid_cameras: # Load frames for the cameras we initialized detectors for
            ...
            if os.path.exists(image_path):
                ...
                except Exception as e: print(f"E: reading {image_path}: {e}"); current_frames_bgr[cam_id] = None
            else: current_frames_bgr[cam_id] = None
        ...
        if valid_annotated:
            ...
            combined_display = np.zeros((rows * target_h, cols * target_w, 3), dtype=np.uint8)
                ...
                for c in range(cols):
                    if frame_num_in_batch < num_cams:
                         ...
                         if frame_to_place.shape[0] != target_h or frame_to_place.shape[1] != target_w:
                              ...
                              except Exception as resize_err: frame_to_place = np.zeros((target_h, target_w, 3), dtype=np.uint8)
