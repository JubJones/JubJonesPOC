...
# --- Added Default Config Values for Pruning ---
...
class MultiCameraPipeline:
    ...
    def __init__(
        ...
        # --- Added State for Pruning ---
        self.global_id_last_seen_frame: Dict[GlobalID, int] = {}
        # Get prune settings from config or use defaults
        self.prune_interval = getattr(config, 'main_gallery_prune_interval_frames', DEFAULT_PRUNE_INTERVAL)
        default_threshold = int(DEFAULT_PRUNE_THRESHOLD_MULTIPLIER * config.lost_track_buffer_frames)
        self.prune_threshold = getattr(config, 'main_gallery_prune_threshold_frames', default_threshold)
        if self.prune_interval > 0:
             ...
             logger.info(" - Main Gallery Pruning: Disabled (interval <= 0).")
    ...
    def _perform_reid_association_batched(
        ...
        # Assign new IDs if no match or filtered out
        for i, track_key in enumerate(query_track_keys):
            if track_key not in tentative_assignments or tentative_assignments[track_key][0] is None:
                ...
                self.global_id_last_seen_frame[new_gid] = self.processed_frame_counter


        # --- 4. Conflict Resolution (Within Camera) ---
        ...
        for track_key, (gid, score) in tentative_assignments.items():
            if track_key not in reverted_keys and gid is not None:
                ...
                self.global_id_last_seen_cam[gid] = track_key[0]
                self.global_id_last_seen_frame[gid] = self.processed_frame_counter

                # Update gallery (EMA) if it was a match
                    ...
                    try:
                        ...
                        if original_gallery_idx != -1 and gallery_types[original_gallery_idx] == 'lost':
                            ...
                            if gid in self.lost_track_gallery:
                                ...
                                logger.debug(f"Moved GID {gid} from lost to main gallery, updated EMA for {track_key}.")
                                ...
                                updated_embedding = (self.config.gallery_ema_alpha * current_gallery_emb +
                                                     (1.0 - self.config.gallery_ema_alpha) * new_embedding)
                                self.reid_gallery[gid] = normalize_embedding(updated_embedding)
                                logger.debug(f"GID {gid} already moved from lost; updated main gallery EMA with {track_key}.")
                            else:
                                logger.warning(f"GID {gid} matched as 'lost' but not found in lost_track_gallery or reid_gallery during finalization for {track_key}. Adding directly to main gallery.")
                            ...
                            updated_embedding = (self.config.gallery_ema_alpha * current_gallery_emb +
                                                 (1.0 - self.config.gallery_ema_alpha) * new_embedding)
                            self.reid_gallery[gid] = normalize_embedding(updated_embedding)
                            logger.debug(f"Updated main gallery EMA for GID {gid} using {track_key}.")
                        # --- END MODIFIED Check ---

                    except IndexError:
                        logger.error(f"IndexError during gallery update for {track_key} -> GID {gid}. Query list length mismatch?")


        # --- 6. Handle Reverted Tracks (Second Pass) ---
        for reverted_key in reverted_keys:
            ...
            for gid, emb in self.reid_gallery.items():
                if gid != conflicting_gid and gid in self.reid_gallery and emb is not None and np.isfinite(emb).all() and emb.size > 0:
                    filtered_gallery_gids.append(gid)
                    filtered_gallery_embeddings.append(normalize_embedding(emb))
            ...
            if filtered_gallery_embeddings:
                ...
                try:
                    ...
                    sims_filtered = np.clip(sims_filtered.flatten(), -1.0, 1.0)
                    ...
                    if sims_filtered.size > 0:
                        ...
                        if max_sim_f >= self.config.reid_similarity_threshold:
                            ...
                            if matched_type_f == 'lost':
                                # --- MODIFIED: Check before pop ---
                                if second_pass_assigned_gid in self.lost_track_gallery:
                                    ...
                                    logger.debug(f"Moved GID {second_pass_assigned_gid} from lost to main gallery via second pass, updated EMA for {reverted_key}.")
                                elif second_pass_assigned_gid in self.reid_gallery:
                                    current_gallery_emb = self.reid_gallery[second_pass_assigned_gid]
                                    updated_embedding = (self.config.gallery_ema_alpha * current_gallery_emb +
                                                         (1.0 - self.config.gallery_ema_alpha) * reverted_embedding_normalized)
                                    self.reid_gallery[second_pass_assigned_gid] = normalize_embedding(updated_embedding)
                                    logger.debug(f"GID {second_pass_assigned_gid} already moved from lost; updated main gallery EMA with {reverted_key} via second pass.")
                                else:
                                    logger.warning(f"Second pass GID {second_pass_assigned_gid} matched as 'lost' but not found in lost or main gallery. Adding directly.")
                                    self.reid_gallery[second_pass_assigned_gid] = reverted_embedding_normalized
                                # --- END MODIFIED Check ---
                                ...
                                updated_embedding = (self.config.gallery_ema_alpha * current_gallery_emb +
                                                     (1.0 - self.config.gallery_ema_alpha) * reverted_embedding_normalized)
                                self.reid_gallery[second_pass_assigned_gid] = normalize_embedding(updated_embedding)
                                logger.debug(f"Updated main gallery EMA for GID {second_pass_assigned_gid} using {reverted_key} via second pass.")
            ...
            if second_pass_assigned_gid is None:
                ...
                new_gid = self.next_global_id; self.next_global_id += 1
                self.reid_gallery[new_gid] = reverted_embedding_normalized
            ...
            self.global_id_last_seen_frame[second_pass_assigned_gid] = self.processed_frame_counter
    ...
    def _update_and_cleanup_state(self, current_frame_active_track_keys: Set[TrackKey]):
        """
        Updates last seen info. Moves disappeared tracks to 'lost_track_gallery',
        purges expired lost tracks, and periodically prunes inactive main gallery tracks.
        """
        # --- Function remains unchanged from previous correct version ---
        current_proc_frame = self.processed_frame_counter

        # Update last seen frame for currently active tracks
        active_global_ids_this_frame: Set[GlobalID] = set()
        for track_key in current_frame_active_track_keys:
            global_id = self.track_to_global_id.get(track_key)
            if global_id is not None:
                self.global_id_last_seen_frame[global_id] = current_proc_frame
        ...
        # Handle disappeared tracks -> move to lost gallery
            ...
            if global_id is not None:
                ...
                if last_feature is not None:
                    if global_id not in self.lost_track_gallery:
                        last_active_frame = self.global_id_last_seen_frame.get(global_id, current_proc_frame - 1)
                        self.lost_track_gallery[global_id] = (last_feature, last_active_frame)
        ...
        expired_lost_gids = [gid for gid, (_, frame_num) in self.lost_track_gallery.items()
                             if (current_proc_frame - frame_num) > self.config.lost_track_buffer_frames]
        if expired_lost_gids:
            logger.info(f"Purging {len(expired_lost_gids)} expired GIDs from lost gallery (Buffer: {self.config.lost_track_buffer_frames} frames).")
            for gid in expired_lost_gids:
                self.lost_track_gallery.pop(gid, None)

        # Periodic Pruning of MAIN Gallery
        if self.prune_interval > 0 and current_proc_frame > 0 and (current_proc_frame % self.prune_interval == 0):
            logger.info(f"--- Performing Main Gallery Pruning (Frame {current_proc_frame}) ---")
            prune_older_than_frame = current_proc_frame - self.prune_threshold
            gids_to_prune: List[GlobalID] = []
            prune_candidates = list(self.global_id_last_seen_frame.keys()) # Iterate over copy

            for gid in prune_candidates:
                last_seen = self.global_id_last_seen_frame.get(gid)
                if last_seen is None: continue

                if last_seen < prune_older_than_frame and gid not in self.lost_track_gallery:
                    gids_to_prune.append(gid)

            if gids_to_prune:
                logger.info(f"Pruning {len(gids_to_prune)} inactive GIDs from main gallery (Threshold: {self.prune_threshold} frames). GIDs: {gids_to_prune[:20]}...")
                for gid in gids_to_prune:
                    ...
                    logger.debug(f"Pruned GID {gid} from main gallery and state.")
            else:
                logger.info("No GIDs met criteria for main gallery pruning.")
            logger.info(f"Main Gallery Size after prune check: {len(self.reid_gallery)}")
        ...
        # Final cleanup of dangling state
        ...
        keys_to_delete_gid_map = set(self.track_to_global_id.keys()) - current_frame_active_track_keys
        for key in keys_to_delete_gid_map: self.track_to_global_id.pop(key, None)
    ...
    def process_frame_batch_full(self, frames: Dict[CameraID, FrameData], frame_idx_global: int) -> ProcessedBatchResult:
        ...
        active_track_keys_this_frame: Set[TrackKey] = set()
        active_triggers_map: Dict[TrackKey, HandoffTriggerInfo] = {}

        for cam_id in self.camera_ids:
            cam_config = self.config.cameras_config.get(cam_id)
            if not cam_config: continue
            ...
            original_frame_bgr = valid_frames_for_processing.get(cam_id)
            ...
            try:
                ...
                if tracked_dets_list is not None and len(tracked_dets_list) > 0:
                    ...
                    if tracked_dets_np_maybe.ndim == 2 and tracked_dets_np_maybe.shape[1] >= 7:
                        tracked_dets_np = tracked_dets_np_maybe[:, :8]
        ...
        for track_key in active_track_keys_this_frame:
            ...
            if cam_output is not None and cam_output.shape[0] > 0:
                try:
                    ...
                    logger.warning(f"Error finding track data row for active key {track_key}: {e}")
                    track_data_row = None

            if track_data_row is None: continue
            ...
            if original_frame_bgr is not None and original_frame_bgr.size > 0:
                ...
                if trigger_reid:
                    ...
                    self.track_last_reid_frame[track_key] = proc_frame_id
        ...
        if tracks_to_extract_features_for:
            ...
            for cam_id, tracks_data_list in tracks_grouped_by_cam.items():
                frame_bgr = valid_frames_for_processing.get(cam_id)
                    ...
                    try:
                        ...
                        for track_id, feature in features_this_cam.items():
                            ...
                            if original_keys_for_tid:
                                extracted_features_this_frame[original_keys_for_tid[0]] = feature
                            else:
                    ...
                    if len(track_data) >= 7:
                        try:
                            ...
                            if homography_matrix is not None:
                                image_point_x = (x1 + x2) / 2.0; image_point_y = y2
                            ...
                            track_info_dict: TrackData = {
                                ...
                                'track_id': track_id, 'global_id': global_id, 'conf': conf,
                                'class_id': cls, 'map_coords': map_coords
                            }
        ...
        # --- Stage 5: Update State and Cleanup (Includes Pruning) ---
        ...
        return ProcessedBatchResult(
            ...
            handoff_triggers=all_handoff_triggers_this_batch
        )